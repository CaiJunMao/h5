...
> 将`相关的元素组织包装起来，提供给程序设计人员使用`，从而降低开发人员的工作量，提供设计Web页面的能力.BOM是一个分层结构。
...
> BOM图谱：
>
> 拓展：Global 对象 是ECMAscript中非常特殊的一个对象，因为这个对象不存在，原因是这个对象无法通过new关键创建。但是这个对象拥有自己的属性、方法甚至全局对象
>
> 一般来说，Global对象默认在浏览器引擎启用时自动初始化完成
>
> 一般来说，Window对象是Global对象的一个子对象，但是在浏览器环境下，Global对象就是指window对象
>
...
> ​	由于 `window 对象同时扮演着 ECMAScript中 Global 对象的角色`，因此所有`在全局作用域中声明`
> `的变量、函数都会变成 **window 对象的属性和方法`**。


> `注意：有三种获取全局变量的方式。全局变量，全局函数，this`
>
>  `var age = 29;	//声明一个全局变量`
...
> - `resizeBy(deltW, deltH) ：增加(参数正数)、缩减（参数负数）指定值的窗口的宽和高`
>
> 一些浏览器已经禁用了这个方法(`Chrome和Firfox均禁用了这两个方法, ie、safarir可用`)
...
> `location对象其实是window对象的子对象`。可以 这样使用：window.location,但是`使用的时候一般省略window`。
>
...
>
> 总结：`有参为true时，从服务器重新下载，无参false从,检测是否改变，未改变从缓存中重新加载，改变从服务器重新下载`
>
...
>
> 在JavaScript中用处不大，只是用来表示客户端的能力，其中包括浏览器窗口外部的显示器的信息，如像素宽度和高度等，各浏览器的screen对象都包含不相同的属性，罗列一些共同的.
...
> `当整个页面加载完成的时会触发该事件`。`用于大型的网页特别是游戏`防止页面（页面加载是耗时操作）未加载时，下面的一些函数就开始运行了，导致取不到一些关键数值
>
> 语法：window.onload = function(){} 或者 window.onload = 方法名
>
> 注意：一般`把事件注册的代码放在head中`。
>
> <script type="text/javascript">
> 	window.onload = function () {//次数window不可省略
> 		alert("页面加载完毕");
> 	}
> </script>
>
...
​	Cookie，有时也用其复数形式 Cookies，指`某些网站为了辨别用户身份、进行 session 跟踪而储存在用户本地终端上的数据（通常经过加密）。`
...
​	cookie机制将信息存储于用户硬盘，因此`可以作为全局变量`，这是它最大的一个优点。它可以用于以下几种场合。 
...
1. `名称`：`一个唯一确定 cookie 的名称。cookie 名称是不区分大小写的`，所以 myCookie 和 MyCookie被认为是同一个 cookie。然而，实`践中最好将 cookie 名称看作是区分大小写的`，因为某些服务器会这样处理 cookie。cookie 的名称必须是经过 URL 编码的。
